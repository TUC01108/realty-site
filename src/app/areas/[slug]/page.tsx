...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!area) return {};
  return { title: `${area.name} Real Estate | Your Big Island Real Estate`, description: area.description };
}
    ...
    <>
      ...
      <section className="mx-auto max-w-6xl px-6 lg:px-8 py-14 lg:py-20 grid gap-12 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2">
            <div>
              <p className="font-display text-coral text-2xl">Top Attractions</p>
              <ul className="mt-3 space-y-2">
                {area.topAttractions.map(item => <li key={item} className="flex items-start gap-2 text-sm text-ink/70"><span className="text-coral mt-0.5">•</span>{item}</li>)}
              </ul>
            </div>
            <div>
              <p className="font-display text-coral text-2xl">Top Restaurants</p>
              <ul className="mt-3 space-y-2">
                {area.topRestaurants.map(item => <li key={item} className="flex items-start gap-2 text-sm text-ink/70"><span className="text-coral mt-0.5">•</span>{item}</li>)}
              </ul>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-6">
          {[["population","Population"],["density","Population Density"],["medianPrice","Median Home Price"]].map(([key,label]) => (
            <div key={key}>
              <p className="font-display text-coral text-3xl">{area[key as keyof typeof area]}</p>
              <p className="eyebrow text-coral mt-1">{label}</p>
              <div className="border-t hairline mt-4" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="grid sm:grid-cols-2 gap-1 mx-auto max-w-6xl px-6 lg:px-8 pb-16 lg:pb-20">
        {[1,2].map(n => (
          <div key={n} className="relative aspect-[4/3] rounded-sm overflow-hidden">
            <Image src={`/images/neighborhoods/${area.slug}-detail-${n}.jpeg`} alt={`${area.name} scenery`} fill className="object-cover" />
          </div>
        ))}
      </section>
      ...
      <section className="bg-coral text-paper">
        <div className="mx-auto max-w-6xl px-6 lg:px-8 py-16 lg:py-20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8">
          <div>
            ...
            <h2 className="font-display text-3xl lg:text-4xl mt-2">Let&apos;s find your home here</h2>
          </div>
          <div className="flex flex-wrap gap-4 shrink-0">
            <Link href="/home-search" className="rounded-sm bg-paper px-6 py-3 text-sm font-medium text-coral hover:bg-transparent hover:text-paper border border-paper transition-colors">Browse Listings</Link>
            <Link href="/contact" className="rounded-sm border border-paper px-6 py-3 text-sm font-medium hover:bg-paper hover:text-coral transition-colors">Contact Yordana</Link>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 lg:px-8 py-16 lg:py-20">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {NEIGHBORHOODS.filter(n => n.slug !== area.slug).slice(0,4).map(n => (
        ...
        <Link href="/areas" className="mt-6 inline-block text-sm font-medium text-coral border-b border-coral pb-0.5 hover:text-coral-dark transition-colors">View all districts</Link>
      </section>
    </>
  );
}
