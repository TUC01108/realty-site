...
export default function BrandBanner() {
  return (
    <section className="relative bg-ink text-paper">
      {/* PHOTO NOTE: PDF uses a dusk luxury-home photo behind this banner */}
      <div className="mx-auto max-w-6xl px-6 lg:px-8 py-20 text-center">
        <p className="font-display text-2xl lg:text-3xl">
          Coldwell Banker Global Luxury&reg;
        </p>
        <p className="eyebrow text-paper/70 mt-2">Guiding You Home Since 1906</p>

        <div className="mt-10 grid gap-8 sm:grid-cols-3">
          {STATS.map((stat) => (
            <div key={stat.label}>
              <p className="font-display text-3xl lg:text-4xl">{stat.value}</p>
              <p className="eyebrow text-paper/70 mt-1">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
